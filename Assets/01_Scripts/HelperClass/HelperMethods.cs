...
public static class HelperMethods
{
    ...
    public static bool GetCompomemtsAtCursorLocation<T>(out List<T> componentsAtPositionList, Vector3 positionToChek)
    {
        bool found = false;
        List<T> componentList = new List<T>();
        Collider2D[] collider2DArray = Physics2D.OverlapPointAll(positionToChek);
        T tComponent = default(T);
        for (int i = 0; i < collider2DArray.Length; i++)
        {
